...
#
# Fixed at 10M records, metric in TS_METRICS (tp|lat, default tp),
# threads 1/2/4/8/16/24 (the dense
...
IFS=' ' read -ra METRICS   <<< "${TS_METRICS:-tp}"
...
for workload in "${WORKLOADS[@]}"; do
  for fixture in "${FIXTURES[@]}"; do
    for metric in "${METRICS[@]}"; do
      ...
      if [ -s "${out}" ]; then
        skip=$((skip + 1))
        echo "SKIP-EXISTS ${fixture} ${workload} ${metric} (${out})"
        continue
      fi
      filter="${fixture}.*${workload}_${metric}.*${THREAD_FILTER}"
      cmd="YCSB_SIZE_TAG=${SIZE}M ${YCSB_BM} --benchmark_filter='${filter}' --benchmark_out='${out}' --benchmark_out_format=json"
      echo "RUN  ${fixture} ${workload} ${metric} → ${out} (timeout ${PER_CELL_TIMEOUT_S}s)"
      ...
      if [ "${rc}" -eq 124 ] || [ "${rc}" -eq 137 ]; then
        ...
        echo "${ts} TIMEOUT ${fixture} ${workload} ${metric}" >> "${TIMEOUT_LOG}"
      elif [ "${rc}" -ne 0 ]; then
        echo "  non-zero exit ${rc} — leaving artifact for inspection"
        echo "$(date +%Y%m%d_%H%M%S) FAIL(${rc}) ${fixture} ${workload} ${metric}" >> "${TIMEOUT_LOG}"
      fi
      echo
    done
  done
